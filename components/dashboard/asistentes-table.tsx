...
type TabValue = "todos" | "queretaro" | "morelia"
...
function matchesTab(asistente: Asistente, tab: TabValue): boolean {
  ...
  const lugar = normalize(asistente.lugar ?? "")
  if (tab === "queretaro") return lugar.includes("queretaro") || lugar.includes("qro")
  if (tab === "morelia") return lugar.includes("morelia")
  return true
}
...
export function AsistentesTable({ data }: Props) {
  ...
  const tabCounts = useMemo<Record<TabValue, number>>(
    () => ({
      todos: data.length,
      queretaro: data.filter((a) => matchesTab(a, "queretaro")).length,
      morelia: data.filter((a) => matchesTab(a, "morelia")).length,
    }),
    [data]
  )
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <AsistenteTriggerButton
          onOpenChange={() => {
            setSelectedAsistente(undefined)
            setIsDialogOpen(true)
          }}
        />
      </div>
